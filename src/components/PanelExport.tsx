...
import type { ExportResolution, OverlaySettings, ChartSettings, ChartData, PostFX, ColorGrade, CoinSettings } from '../types'
...
import { SectionLabel, CheckRow, ToggleGroup, PrimaryBtn, DimBtn, Advanced, Slider } from './ui'
...
const RES_OPTS: { value: ExportResolution; label: string }[] = EXPORT_RESOLUTIONS.map(r => ({ value: r.value as ExportResolution, label: r.label }))
...
export function PanelExport({ engine, coin, overlay, chart, chartData, fx, grade, bgColor, bgImg }: Props) {
  ...
  async function savePngSeq() {
    ...
    for (let i = 0; i < total; i++) {
      // Advance the engine's internal angle by poking it directly isn't exposed,
      // so we grab the current frame — user should let it spin through or pause.
      // For a proper sequence we render at fixed angles via a manual approach:
      const frame = engine.snapshot(transparent, bgColor, bgImg, fx, grade)
  ...
  function savePost() {
    ...
    // Overlay + chart
    drawOverlay(ctx, W, H, overlay, chart, chartData, defaultConfetti(), { ...defaultChartAnim(), duration: 0 }, createAnimState())
  ...
  function toggleRecord() {
    ...
    const mr = new MediaRecorder(fxCanvas.captureStream(60), { mimeType: mime, videoBitsPerSecond: 12_000_000 })
    const chunks: Blob[] = []
    ...
    mr.onstop = () => {
      ...
      const a = document.createElement('a')
      a.href = URL.createObjectURL(blob); a.download = 'coin.webm'; a.click()
      URL.revokeObjectURL(a.href)
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1">
        ...
        <ToggleGroup options={RES_OPTS} value={resolution} onChange={setResolution} cols={3} />
      </div>
      ...
      <Advanced>
        <Slider label="Rec. Duration" min={1} max={20} step={1} value={recDuration} display={v => Math.round(v) + 's'} onChange={v => setRecDuration(Math.round(v))} />
        <Slider label="Seq. Frames"  min={12} max={120} step={4} value={seqFrames} display={v => Math.round(v).toString()} onChange={v => setSeqFrames(Math.round(v))} />
        <p className="text-sm text-neutral-500">ffmpeg -i coin.webm -c:v libx264 coin.mp4</p>
        <p className="text-sm text-neutral-500">ffmpeg -r 30 -i coin_%04d.png -vf fps=30 coin.gif</p>
      </Advanced>
    </div>
  )
}
