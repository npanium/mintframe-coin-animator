  ...
  return (
    <div className="flex items-center gap-2 text-sm">
      ...
      <input
        ...
        className="flex-1 accent-amber-500 cursor-pointer h-0.5"
      />
    ...
    <div className={`grid gap-1`} style={{ gridTemplateColumns: `repeat(${cols}, 1fr)` }}>
      {options.map(o => (
        <button key={o.value} onClick={() => onChange(o.value)}
          className={`py-1.5 text-sm rounded border transition-all cursor-pointer ${value === o.value
...
// ── Text input ──
export function TextInput({ label, value, onChange, large }: { label: string; value: string; onChange: (v: string) => void; large?: boolean }) {
  return (
...
export function Advanced({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {open && <div className="flex flex-col gap-3 pl-1 border-l border-neutral-800">{children}</div>}
    </div>
  )
}
...
// ── FX toggle button ──
export function FxToggle({ label, active, onClick }: { label: string; active: boolean; onClick: () => void }) {
  return (
    <button onClick={onClick} className={`py-1.5 text-sm rounded border transition-all cursor-pointer ${
      active ? 'border-blue-500 text-blue-400 bg-blue-950' : 'border-neutral-800 text-neutral-500 bg-neutral-950 hover:border-neutral-600 hover:text-neutral-300'
    }`}>{label}</button>
  )
}
...
// ── Dim button ──
export function DimBtn({ children, onClick, disabled }: { children: ReactNode; onClick: () => void; disabled?: boolean }) {
  return (
    <button onClick={onClick} disabled={disabled}
